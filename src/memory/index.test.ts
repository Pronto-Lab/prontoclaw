...
let getMemorySearchManager: GetMemorySearchManager;
...
describe("memory index", () => {
  ...
  it("keeps dirty false in status-only manager after prior indexing", async () => {
    ...
    if (!first.manager) {
      throw new Error(first.error ?? "manager missing");
    }
    ...
    if (!statusOnly.manager) {
      throw new Error(statusOnly.error ?? "status manager missing");
    }
  ...
  it("reindexes when the embedding model changes", async () => {
    ...
    if (!second.manager) {
      throw new Error(second.error ?? "manager missing");
    }
