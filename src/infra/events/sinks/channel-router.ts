...
async function executeListGuildChannels(
  guildId: string,
  discordOpts: { accountId: string },
): Promise<string> {
  ...
  const textChannels = channels
    ...
    .map((ch) => ({
      ...
      category: "parent_id" in ch && ch.parent_id ? (categories.get(ch.parent_id) ?? null) : null,
    }));
  return JSON.stringify(textChannels);
}

async function executeListActiveThreads(
  guildId: string,
  discordOpts: { accountId: string },
): Promise<string> {
...
async function executeReadRecentMessages(
...
async function executeTool(
  ...
  switch (toolName) {
    case "listGuildChannels":
      return executeListGuildChannels(guildId, discordOpts);
    case "listActiveThreads":
      return executeListActiveThreads(guildId, discordOpts);
    case "readRecentMessages":
      return executeReadRecentMessages(args, discordOpts);
    default:
...
export class ChannelRouter {
  ...
  async route(context: RouteContext): Promise<RouteResult> {
    ...
    try {
      const result = await this.doRoute(context, controller.signal);
      return result ?? this.buildFallback(context);
    } catch (err) {
  ...
  private async doRoute(context: RouteContext, signal: AbortSignal): Promise<RouteResult> {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        const result = await this.runToolLoop(resolved.model, llmContext, apiKey, signal);
        if (result) {
          return result;
        }
      } catch (err) {
  ...
  private async runToolLoop(
    ...
    for (let turn = 0; turn <= MAX_TOOL_TURNS; turn++) {
      ...
      const { provider } = parseModelString(this.routerModel);
  ...
  private buildFallback(context: RouteContext): RouteResult {
    const threadName = `[협업] ${context.fromAgentName} · ${context.toAgentName}`.slice(
      0,
      THREAD_NAME_MAX,
    );
    return {
