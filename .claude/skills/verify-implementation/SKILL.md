...
실행 대상 스킬 테이블의 내용을 표시하고 순차 실행합니다.

### Step 2: 순차 실행

**실행 대상 스킬** 테이블에 나열된 각 스킬에 대해:

#### 2a. 스킬 SKILL.md 읽기

해당 스킬의 `.claude/skills/verify-<name>/SKILL.md`를 읽고 Workflow, Exceptions, Related Files 섹션을 파싱합니다.
...
이슈가 발견된 경우 `AskUserQuestion`을 사용하여 수정 옵션을 확인합니다:
