...
등록된 스킬이 0개인 경우, Step 4 (CREATE vs UPDATE 결정)로 바로 이동합니다. 모든 변경 파일이 UNCOVERED로 처리됩니다.
...
1. **Lock 파일 및 생성된 파일** — `package-lock.json`, `pnpm-lock.yaml` 등은 스킬 커버리지가 불필요
2. **일회성 설정 변경** — `package.json`의 버전 범프 등은 새 스킬이 불필요
3. **문서 파일** — `README.md`, `CHANGELOG.md`, `LICENSE` 등은 검증이 필요한 코드 패턴이 아님
4. **테스트 픽스처 파일** — 테스트 픽스처 디렉토리의 파일은 프로덕션 코드가 아님
5. **영향받지 않은 스킬** — UNAFFECTED로 표시된 스킬은 검토 불필요
6. **CLAUDE.md 자체** — 문서 업데이트이며, 검증이 필요한 코드 패턴이 아님
7. **벤더/서드파티 코드** — `vendor/`, `node_modules/` 디렉토리의 파일은 외부 규칙을 따름
8. **CI/CD 설정** — `.github/`, `Dockerfile` 등은 인프라
