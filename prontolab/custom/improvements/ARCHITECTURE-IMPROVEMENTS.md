...
이 문서는 prontolab-openclaw 코드베이스의 종합 아키텍처 리뷰에서 도출된 **10개 핵심 개선안**의 마스터 플랜입니다.
각 개선안은 별도 상세 설계 문서(`improvements/01-*.md` ~ `improvements/10-*.md`)로 관리됩니다.
...
**총 노력 추정**: XL (전체 클린하게 수행 시 약 3-4주)
...
1. **#1 A2A 대화 인덱스** — 적은 노력으로 즉각적인 성능 개선
2. **#3 task-tool 분할** — 적은 노력으로 유지보수성 대폭 향상
3. **#5 Gateway 조합** — 적은 노력으로 머지 충돌 감소
4. **#2 A2A 내구성** — 노력은 크지만 시스템 안정성의 근본적 개선
5. **#7 A2A 동시성** — 에이전트 수 증가 시 필수
...
| Phase | 성공 기준 |
|-------|-----------|
| Phase 1 | `pnpm test` 통과, `pnpm build` 통과, A2A 대화 연속성 O(1) 확인, task-tool 모듈 분리 확인 |
| Phase 2 | Gateway 재시작 후 A2A 플로우 복구 확인, DI 서비스 경계 동작 확인 |
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-02-19 | 초기 문서 작성 (아키텍처 리뷰 기반) |
