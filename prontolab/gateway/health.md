...
## ProntoLab deployment note

- In this ProntoLab deployment, HTTP probes on gateway port `18789` (`/`, `/health`, `/api/health`) currently return `404`.
- Use WS/RPC health commands (`openclaw status --deep`, `openclaw health --json`) as primary gateway checks.
- For HTTP liveness, use Task Monitor: `http://127.0.0.1:3847/api/health`.
