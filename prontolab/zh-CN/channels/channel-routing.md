...
- Slack 线程会在基础键后追加 `:thread:<threadTs>`。
- Discord 线程会直接使用线程频道 ID 作为 channel peer 键。
- Telegram 论坛主题在群组键中嵌入 `:topic:<topicId>`。

示例：

- `agent:main:telegram:group:-1001234567890:topic:42`
- `agent:main:slack:channel:C123456:thread:1710000000.123456`
- `agent:main:discord:channel:987654321012345678`

Discord 说明：线程继承关系通过父会话元数据（例如 `ParentSessionKey`）关联，而不是在 session key 后追加 `:thread:<id>`。
