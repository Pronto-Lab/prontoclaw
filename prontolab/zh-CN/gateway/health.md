...
- 在当前 ProntoLab 部署中，Gateway 网关端口 `18789` 上的 HTTP 探测（`/`、`/health`、`/api/health`）目前返回 `404`。
- Gateway 网关健康检查请优先使用 WS/RPC 命令（`openclaw status --deep`、`openclaw health --json`）。
- 若需要 HTTP 存活检查，请使用 Task Monitor：`http://127.0.0.1:3847/api/health`。
